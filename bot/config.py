...
# ─────────────────────────────────────────
# 진입 조건
MIN_PRICE_MOVE_PCT        = 0.3    # 바이낸스 가격이 30초 내 최소 0.3% 움직여야 신호 발생
MIN_GAP_PERCENTAGE_POINTS = 3.0    # 폴리마켓 오즈와 실제 확률 괴리 최소 3%p 이상
PRICE_WINDOW_SEC          = 30     # 가격 변동 감시 시간창 (초)

# 청산 조건
MAX_HOLD_TIME_SEC   = 8     # 진입 후 최대 보유 시간 (초) - 초과 시 강제 청산
ODDS_CONVERGENCE_PCT = 0.015  # 오즈 수렴 감지 임계치 (1.5%p 수렴 시 청산)
STOP_LOSS_PCT       = 0.02   # 오즈가 역방향으로 2%p 이상 벌어지면 손절

# 유동성 체크
MIN_MARKET_LIQUIDITY_USD = 50_000   # 최소 시장 유동성 $50,000
